...
upload_endpoint = base_endpoint + "upload"
transcript_endpoint = base_endpoint + "transcript"
...
def get_response(polling_endpoint, header):
    #paragraphs_response = requests.get(polling_endpoint + "/paragraphs", headers=header)
    response = requests.get(polling_endpoint, headers=header)
    response = response.json().get("sentiment_analysis_results")

    speakers = []
    for sentiments in response:
        x = {
            "text": sentiments['text'],
            "start": sentiments['start'],
            "end": sentiments['end'],
            "speaker": sentiments['speaker'],
